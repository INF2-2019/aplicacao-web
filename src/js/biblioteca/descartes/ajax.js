...
const links = {
  consulta: "/biblioteca/descartes/consulta",
  inserir: "/biblioteca/descartes/inserir"
};

const container_resultados = document.querySelector("#holder_descartados");
function pegaTabela(link) {
    return (fetch(prefixo+link).then(res => res.text()));
}

async function mostraAcervosDescartados(){
  let xml_string = await pegaTabela(link.consulta);
